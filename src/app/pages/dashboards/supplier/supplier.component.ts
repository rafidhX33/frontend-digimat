  ...
  goBackToSupplier(): void {
    ...
    checkDocumentStatus(kode_vendor: string, index: number): void {
     ...
     /** 🔹 Navigasi ke halaman Upload Dokumen */
  navigateToUpload(kode_vendor: string): void {
    this.router.navigate(["/pdf"], { queryParams: { supplier: kode_vendor } });
  }

  /** 🔹 Navigasi ke halaman Logo & Status */
  navigateToSupplierInfo(kode_vendor: string): void {
    this.router.navigate(["/supplier-info"], { queryParams: { supplier: kode_vendor } });

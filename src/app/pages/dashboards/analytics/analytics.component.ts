...
export class AnalyticsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const now = new Date();
    this.kedatanganStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().substring(0, 10);
    this.kedatanganEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString().substring(0, 10);
  }
  ...
  loadSupplierData(kode_vendor: string): void {
    ...
    this.http.get<any>(urlGrade).subscribe(
      response => {
        if (response.success && Array.isArray(response.data) && response.data.length > 0) {
          this.selectedSupplier = { ...this.selectedSupplier!, supplierScore: response.data[0].Grade_Material || "-" };
        } else {
          this.selectedSupplier = { ...this.selectedSupplier!, supplierScore: "-" };
        }
      },
      error => {
    ...
    this.http.get<any>(urlComplaint).subscribe(
      response => {
        if (response.success) {
          this.selectedSupplier = { ...this.selectedSupplier!, complaintCount: response.data.total_complaints || 0 };
        } else {
          this.selectedSupplier = { ...this.selectedSupplier!, complaintCount: 0 };
        }
      },
      error => {
    ...
    this.http.get<any>(urlSupplierDetails).subscribe(
      response => {
        if (response.success && response.data) {
          this.selectedSupplier = { ...this.selectedSupplier!, image: response.data.image || "assets/default-logo.png", status: response.data.status || "Unknown" };
        }
      },
      error => {
    ...
    this.http.get<any>(urlSupplierInfo).subscribe(
      response => {
        if (response.success && response.data) {
          ...
          this.selectedSupplier = { ...this.selectedSupplier!, image: logoUrl, status: response.data.status || "Unknown" };
        }
      },
      error => {
...
loadSupplierDocs(kode_vendor: string, material_code: string): void {
  ...
  this.http.get<any>(url).subscribe(
    response => {
      ...
      filteredDocs.forEach(doc => {
        const cleanPath = (path: string | null) => path ? path.replace(/\\/g, '/') : null; // Konversi semua backslash ke slash
      ...
      response => {
        if (response.success && Array.isArray(response.data)) {
          this.supplierAbnormalities = response.data.map((item: any) => ({
            ...
            date: item.date_occurrence ? new Date(item.date_occurrence) : new Date()
          }));
  ...
  loadKedatangan(kode_vendor: string, material_code: string, startDate: string, endDate: string): void {
    const url = `${this.apiUrl}/api/kedatangan?supplier=${kode_vendor}&material=${material_code}&start=${startDate}&end=${endDate}`;
    this.http.get<any>(url).subscribe(
      response => {
        if (response.success && Array.isArray(response.data)) {
          const filtered = response.data.filter((item: KedatanganData) =>
  ...
  get totalAbnormalityPages(): number {
    return Math.ceil(this.supplierAbnormalities.length / this.abnormalityPageSize);
  }
  get paginatedAbnormalities(): Abnormality[] {
    const startIndex = (this.abnormalityPage - 1) * this.abnormalityPageSize;
    return this.supplierAbnormalities.slice(startIndex, startIndex + this.abnormalityPageSize);
  }
  ...
  get paginatedKedatangan(): KedatanganData[] {
    ...
    return this.kedatanganData.slice(startIndex, startIndex + this.kedatanganPageSize);
  }
